...
//Эти функции реализуют метод наименьших квадратов для нахождения полинома второй степени (параболы y = ax^2 + bx + c), который наилучшим образом описывает набор входных данных data.
//S_X: Предварительно вычисляет суммы степеней x (от 1 до 4) и определитель основной матрицы системы уравнений. Это делается для оптимизации, чтобы не пересчитывать одни и те же значения многократно.
//predict: Находит коэффициенты a, b, c для параболы, решая систему линейных уравнений по формулам Крамера. После нахождения коэффициентов функция делает прогноз значения y для следующей точки x = N.
//4. Калибровка
